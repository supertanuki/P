...
function pageheader($title, $options = false)
{
    ...
    <div id="header_links">
        <ul>
            <?php if (!$user) { ?>
                <li class="header_link_orange"><a id="login_link" href="javascript:" onclick="SeConnecter(this);">Se
                        connecter</a></li>
                <li class="header_link_blue" id="btn_inscrire"><a href="javascript:" onclick="Sinscrire(this);">S'inscrire</a>
                </li>
            <?php
            } else {
                ...
                if ($avatar = getAvatar($user->id_user, $user->avatar_key, $user->avatar_ext, 'small')) {
                    ?>
                    <div style="float:left;"><img src="/avatars/<?php echo $avatar ?>" height="29" width="29" border="0"
                                                  align="absmiddle" style="border-right:solid 1px #fff"/></div><?php
                }
        ...
        <div
            style="display:block; width:300px; height:12px; overflow:hidden; color:#666666; font-size:12px; font-weight:bold; margin:0; padding:5px 5px 2px 5px;">
            <?php echo($title == "Prono+, Pronostic de foot" ? "Jeu gratuit de pronostics de football" : str_replace(
                '| Prono+',
                '',
                $title
            )); ?>
...
<?php } ?>
    ...
    if (navigator_is_mobile()) {
        ?>
        <p align="center"
           style="clear:left; padding: 10px; background: none repeat scroll 0% 0% rgb(255, 255, 170); font-size:24px;">
            <b>
                Tu es sur ton mobile ?<br/><a href="/mobile/" class="link_orange">Utilise la version mobile de
                    Prono+</a> !
            </b></p>
    <?php
    } else {
        ?>
        <noscript><p align="center" style="padding: 10px; background: none repeat scroll 0% 0% rgb(255, 255, 170);"><b>Ce
                    site nécessite JavaScript pour fonctionner intégralement.<br> Merci de l'activer dans votre
                    navigateur.</b></p></noscript>
    <?php
    }
    ?>
<?php
              ...
              LIMIT 1";
                    ...
                    if (DB::isError($result_cup_matches)) {
                        ...
                        if ($pp_cup_matches = $result_cup_matches->fetchRow()) {
                            ...
                            $html .= "<li><a href=\"/cup.php?id=" . $pp_cup_matches->id_cup . "\">" . formatDbData(
                                    $pp_cup_matches->label
                                ) . " - " . getCupTourLabel($pp_cup_matches->number_tour) . "</a></li>";
                        }
                    }

                } else {
                    $html .= "<li><a href=\"/classj.php?id=" . $pp_matches->id_matches . "\">" . formatDbData(
                            $pp_matches->label
                        ) . "</a></li>";
                }
            }
...
function listDerniersResultats()
{
    ...
    if (DB::isError($result_matches)) {
        ...
        if ($result_matches->numRows()) {
            ...
            if ($html) {
                $html = '<strong>Les derniers résultats</strong><ul>' . $html . '</ul>';
            }
        }
    }
...
function pagefooter($options = false)
{
	...
	<?php }*/
        ?>
        <img src="/template/default/whosonline.png" height="25" width="37" border="0" alt="" align="absmiddle"
             style="margin-right:6px;"/><strong>Qui est en ligne ?</strong>
        ...
        if (DB::isError($result_pp_user)) {
            ...
            while ($pp_user = $result_pp_user->fetchRow()) {
                ...
                $content .= "<a href=\"/user.php?q=" . urlencode(
                        htmlspecialchars($pp_user->login)
                    ) . "\" class=\"link_button\" style=\"margin:0 4px 4px 0\"><img src=\"" . $avatar . "\" height=\"29\" width=\"29\" border=\"0\" align=\"absmiddle\" />&nbsp;" . str_replace(
                        ' ',
                        '&nbsp;',
                        $pp_user->login
                    ) . "</a>";
        ...
        if ($content == '') {
            ...
            echo $nb_online . " joueur" . ($nb_online > 1 ? "s" : "") . " en ligne<hr>" . $content;
        }
        ?>
        <div class="clear"></div>
    </div>
            ...
            <tr>
                <td valign="top" width="33%">
                    ...
                    <ul>
                        ...
                        <?php if ($user->id_user) { ?>
                            <li><a href="/user.php?q=<?php echo htmlspecialchars(urlencode($user->login)); ?>">Mon
                                    profil</a></li>
                        ...
                        <?php } else {
                            if (!$options['forblog']) {
                                ...
                                <li><a href="javascript:" onclick="Sinscrire(this);">S'inscrire gratuitement à
                                        Prono+</a>
                                </li>
                            <?php
                            }
		...
		ORDER BY `pp_class`.`order`";
                    ...
                    if (DB::isError($result)) {
                        ...
                        while ($pp_class = $result->fetchRow()) {
                            echo "<li><a href=\"/class.php?id=" . $pp_class->id_class . "\" class=\"link_orange\">" . formatDbData(
                                    $pp_class->label
                                ) . "</a></li>";
                        }
                        echo "</ul>";
          ...
          ORDER BY `ordre`";
                ...
                <td valign="top" width="33%">
                    ...
                    <ul>
                        <li><a href="http://www.richardhanna.fr/"
                               title="Développeur Web, Web Mobile, PHP/Mysql, Symfony2, Wordpress">Développeur Web, Web
                                Mobile, PHP/Mysql, Symfony2, Wordpress</li>
                    </ul>
    ...
    if (DB::isError($result)) {
        ...
        while ($pp_class_autres = $result->fetchRow()) {
            if ($type_current != $pp_class_autres->type) {
                ...
                if ($type_current == 'year') {
                    $html .= ($html ? "</ul><br />" : "") . "<b>Classements annuels</b><ul>";
                } else {
                    if ($type_current == 'month') {
                        $html .= "<ul><li><a href=\"/classements.php\" class=\"link_orange\">Tous mes classements</a></li></ul><br /><b>Classements mensuels</b><ul>";
                    }
                }
            }
            if ($html) {
                $html .= "<li><a href=\"/class.php?id=" . $pp_class_autres->id_class . "\" class=\"link_orange\">" . formatDbData(
                        $pp_class_autres->label
                    ) . "</a></li>";
            }
        }
        if ($html) {
          ...
          ORDER BY `cup_sub`";
            ...
            if (DB::isError($result_division)) {
                ...
                while ($cup_division = $result_division->fetchRow()) {
                    $html .= "<li><a href=\"/cup.php?id=" . $pp_cup_matches->id_cup . "&division=" . $cup_division->cup_sub . "\">" . getCupDivisionLabel(
                            $cup_division->cup_sub
                        ) . "</a></li>";
                }
                $html .= "</ul>";
            }
...
function getOnglets($onglet_actif = false)
{
    ...
    $onglets[] = array(
        ...
        'title' => 'Prono+',
        'url' => '/',
        'richcontent' => listGrillesEnCours() . '<br />' . listDerniersResultats()
    );
    ...
    $onglets[] = array(
        ...
        'title' => 'Classements',
        'url' => '/classements.php',
        'alt' => 'Tous les classements de Prono+',
        'richcontent' => listClassements()
    );
    ...
    $onglets[] = array(
        ...
        'url' => '#',
        'alt' => '',
        'richcontent' => listStatsMenu()
    );

    $onglets[] = array(
        'id' => 'agenda',
        'title' => 'Agenda',
        'url' => '/agenda-football.php?date=' . date("d-m-Y"),
        'alt' => 'Agenda football'
    );
    ...
    if ($user) {
        $onglets[] = array(
            'id' => 'mon_profil',
            'title' => 'Mur' . $html_nb_msg_wall,
            'url' => '/user.php?q=' . htmlspecialchars(urlencode($user->login)),
            'alt' => $alt_msg_wall ? $alt_msg_wall : 'Mon profil et mon mur'
        );
    }

    $onglets[] = array(
        'id' => 'mes_amis',
        'title' => 'Amis' . $html_nb_msg_friends,
        'url' => '/friends.php',
        'alt' => $alt_msg_friends ? $alt_msg_friends : 'Mes amis et leurs activités'
    );
    ...
    if ($user) {
        $onglets[] = array(
            'id' => 'mes_recompenses',
            'title' => 'Etoiles',
            'url' => '/recompenses.php?q=' . htmlspecialchars(urlencode($user->login)),
            'alt' => 'Mes récompenses'
        );
    }
    ...
    if ($onglet_actif == 'equipe-de-france') {
        $html .= "<div id=\"edf_header_onglet\">";
    }
    ...
    if (is_array($onglets)) {
        foreach ($onglets as $onglet) {
            ...
            if ($onglet['richcontent'] != '') {
                $html .= '<div class="submenu_onglet">' . $onglet['richcontent'] . "</div>";
            }
            $html .= "</li>";
        }
    }
...
*/
				...
				*/
                ...
                <li>
                    <div class="list_handle"><h2 class="title_blueking">Tu n'es pas fan de Prono+ sur Facebook ? Non
                            mais allô quoi !</h2></div>
                    <div class="bloc_content">
                        <iframe
                            src="//www.facebook.com/plugins/likebox.php?href=https%3A%2F%2Fwww.facebook.com%2Fpronoplus&amp;width=676&amp;height=558&amp;colorscheme=light&amp;show_faces=true&amp;header=false&amp;stream=true&amp;show_border=false&amp;appId=302697239001"
                            scrolling="no" frameborder="0"
                            style="border:none; overflow:hidden; width:676px; height:558px;"
            ...
            if ($pp_alaune->type == 'message') {
                ...
                $posts = PreviewBlog(
                    array(
                        'type' => 'alaune',
                        'days_expire' => 7,
                        'nbelements' => $pp_alaune->message ? $pp_alaune->message : 3
                    )
                );
                if (is_array($posts)) {
                ...
                if ($pp_alaune->type == 'matches' && !$user->id_user) {
                    // matchs mis en avant ?
                    $SQL = "SELECT `pp_matches`.`id_matches`, `pp_matches`.`label`, `pp_matches`.`id_cup_matches`,
                        ...
                        while ($pp_matches = $result_matches->fetchRow()) {
                            ...
                            if ($pp_matches->id_cup_matches) {
                                ...
                                if (!DB::isError($result_cup)) {
                                    while ($pp_cup_matches = $result_cup->fetchRow()) {
                                        $pp_matches->id_cup = $pp_cup_matches->id_cup;
                                    }
                                }
                            }
                            ...
                            $dayweek = $dayweek - 2;
                            $pp_matches->date_first_match = get_date_complete(
                                    $dayweek,
                                    $pp_matches->date_first_match_day,
                                    $pp_matches->date_first_match_month - 1,
                                    $pp_matches->date_first_match_year
                                ) . " &agrave; " . $pp_matches->date_first_time_match_format;
                            ...
                            $content .= $link . "<img src=\"/template/default/" . $pp_matches->image . "\" class=\"preview_matches_image\" border=\"0\" style=\"border:solid 3px #eee;\" alt=\"" . htmlspecialchars(
                                    formatDbData($pp_matches->label)
                                ) . "\" /></a>";
                            $content .= "<h2>" . $link . formatDbData($pp_matches->label) . "</a></h2>";
                            $content .= "<span class=\"red\">" . format_diff_date(
                                    $pp_matches->diff_date_first_match
                                ) . "</span></strong>";
                            ...
                            if (DB::isError($result_match)) {
                                ...
                                while ($pp_match = $result_match->fetchRow()) {
                                    $content .= "<tr>
										<td align=\"right\" " . ($pp_match->diff_date_match > 0 ? "class=\"strike\"" : "") . " width=\"49%\">" . formatDbData(
                                            $pp_match->team_host_label
                                        ) . "</td>
										<td>-</td>
										<td " . ($pp_match->diff_date_match > 0 ? "class=\"strike\"" : "") . "  width=\"49%\">" . formatDbData(
                                            $pp_match->team_visitor_label
                                        ) . "</td>
									</tr>";
                                }
                                $content .= "</table>";
            ...
            <div class="bulle_content">
                <div class="center">
                    ...
                    if ($userprofil->id_user == $user->id_user) {
                        $form = '<div id="bulle_edit_text" style="display:none"><form method="post" action="/user.php?q=' . urlencode(
                                htmlspecialchars($userprofil->login)
                            ) . '">
						...
						<input type="hidden" name="pp_comments_key" value="' . md5(
                                'wall_pronoplus_' . $userprofil->id_user . '_pronoplus_'
                            ) . '" />
...
function getRightProfil($pp_user = 0)
{
    ...
    <div class="profil">
        ...
        <?php if (!$avatar && $userprofil->id_user && $user->id_user == $userprofil->id_user) { ?>
        <div class="message_error"><?php } ?>
            ...
            <?php if (!$avatar && $userprofil->id_user && $user->id_user == $userprofil->id_user) { ?><br/><a
                href="/avatar.php">Choisis maintenant un avatar qui te représente !</a></div><?php } ?>

        <div class="pseudo_joueur">
            <?php if ($userprofil) { ?>
                <?php echo formatDbData(
                    $userprofil->login
                ) ?> <?php /*<img src="template/default/status_online.png" alt="En ligne" title="En ligne" height="16" width="16" />*/ ?>
                <br/><p class="<?php echo $nb_recompenses != 0 ? 'recompense-star' : 'recompense-star-disabled'; ?>"><a
                        href="/recompenses.php?q=<?php echo urlencode(
                            $userprofil->login
                        ); ?>"><?php echo $nb_recompenses; ?> <?php echo $nb_recompenses > 1 ? 'étoiles' : 'étoile'; ?></a>
                </p>
                <br/>
                <small>Inscrit(e) depuis le <?php echo $userprofil->register_date_format; ?></small>
        ...
        <?php if ($user->id_user && $userprofil->id_user == $user->id_user) { ?>
            <div class="btn_ajouter_contenu" align="center">
                <a href="/profil.php"><img src="template/default/user_edit.png" align="absmiddle"
                                           alt="Modifier mon profil" height="16" width="16" border="0"/> Modifier mon
                    profil</a>
            </div>
        <?php } ?>
        ...
        if ($user->id_user && $userprofil->id_user == $user->id_user) {
            ...
            if ($pp_user_friends = $result_user_friends->fetchRow()) if ($pp_user_friends->nb_invitations) {
                ...
                <br/>
                <div align="center">
                    <a class="link_button_urgent" href="/friends.php"><img src="/template/default/group_add.png"
                                                                           height="16" width="16" border="0"
                                                                           align="absmiddle"/> <?php echo $pp_user_friends->nb_invitations . ' invitation' . ($pp_user_friends->nb_invitations > 1 ? 's' : '') . ' d\'ami(e)' . ($pp_user_friends->nb_invitations > 1 ? 's' : ''); ?>
                    </a>
                </div>
            <?php
            }
        ...
        if ($user->id_user || $userprofil) {
            ...
            <div class="profil_palmares_border">
                <table cellspacing="0" cellpadding="2" align="center">
                    <tr>
                        <td align="right" width="70%"><a href="class.php?id=2&rech_jpseudo=<?php echo urlencode(
                                $userprofil->login
                            ) ?>&search_joueur=1">Classement G&eacute;n&eacute;ral</a> :
                        </td>
                        <td width="30%" nowrap>
                            <?php
                            if ($class_user[2]['class']) {
                                ?>
                                <a href="class.php?id=2&rech_jpseudo=<?php echo urlencode(
                                    $userprofil->login
                                ) ?>&search_joueur=1"><?php echo $class_user[2]['class'] ?>
                                    <?php
                    ...
                    if (DB::isError($result_class)) {
                        ...
                        if ($pp_class = $result_class->fetchRow()) {
                            ...
                            <tr>
                                <td align="right"><a
                                        href="class.php?id=<?php echo $pp_class->id_class ?>&rech_jpseudo=<?php echo urlencode(
                                            $userprofil->login
                                        ) ?>&search_joueur=1"><?php echo $pp_class->label ?></a> :
                                </td>
                                <td nowrap>
                                    <?php
                                    if ($class_user[$pp_class->id_class]['class']) {
                                        ?>
                                        <a href="class.php?id=<?php echo $pp_class->id_class ?>&rech_jpseudo=<?php echo urlencode(
                                            $userprofil->login
                                        ) ?>&search_joueur=1"><?php echo $class_user[$pp_class->id_class]['class'] ?>
                                            <?php
                ...
                <ul>
                    ...
                    <li><a href="/recompenses.php?q=<?php echo urlencode(htmlspecialchars($userprofil->login)); ?>">Récompenses</a>
                    </li>
                    <li><a href="/palmares-archives.php?q=<?php echo urlencode(
                            htmlspecialchars($userprofil->login)
                        ); ?>">Archives palmarès</a></li>
                </ul>
...
function getRandomFriends($pp_user)
{
    ...
    if ($nb_friends = $result_user_friends->numRows()) {
        while ($pp_user_friends = $result_user_friends->fetchRow()) {
            $avatar = getAvatar(
                $pp_user_friends->id_user,
                $pp_user_friends->avatar_key,
                $pp_user_friends->avatar_ext,
                'small'
            );
            $avatar = $avatar ? '/avatars/' . $avatar : '/template/default/_profil.png';
            $html .= '<a href="/user.php?q=' . urlencode(
                    htmlspecialchars($pp_user_friends->login)
                ) . '" class="link_button" style="margin:0 4px 4px 0"><img src="' . $avatar . '" height="29" width="29" border="0" align="absmiddle" />&nbsp;' . str_replace(
                    ' ',
                    '&nbsp;',
                    $pp_user_friends->login
                ) . '</a>';
        }
...
function getContentRight()
{
    ...
    if ($user->id_user) {
        ...
        <li>
            ...
            <div class="bloc_content">
                ...
                <hr/>
                <div style="text-align:center"><a href="/friends.php" class="link_orange">Activités de mes amis</a>
                </div>
            </div>
        </li>
    ...
    <li>
        ...
        <div class="bloc_content">
            ...
            while ($pp_newuser = $result_newusers->fetchRow()) {
                ...
                $html .= '<a href="/user.php?q=' . urlencode(
                        htmlspecialchars($pp_newuser->login)
                    ) . '" class="link_button" style="margin:0 4px 4px 0"><img src="' . $avatar . '" height="29" width="29" border="0" align="absmiddle" />&nbsp;' . str_replace(
                        ' ',
                        '&nbsp;',
                        $pp_newuser->login
                    ) . '<br /><span class="petitgris">Le&nbsp;' . $pp_newuser->register_date_format . '</span></a>';
            }
            $html .= '<div style="clear:both;"></div>';
            echo $html;
            ?>
    ...
    if (!$user->id_user) {
        ...
        if (DB::isError($result_match)) {
            ...
            while ($pp_match = $result_match->fetchRow()) {
                $content[] = '<a href="/pronostiquer.php?id=' . $pp_match->id_matches . '" class="link_orange">Pronostic ' . formatDbData(
                        $pp_match->team_host_label
                    ) . ' - ' . formatDbData($pp_match->team_visitor_label) . '</a>';
            }
        }
        if (count($content)) {
            ?>
    ...
    <li>
        ...
        <div class="bloc_content">
            <p>Prono+ existe depuis 2000 et ne rapporte pas grand chose à son webmaster. Ce site est un investissement
                important en temps et en frais d'hébergement (environ 230€/an serveur dédié). Si vous souhaitez aider
                Prono+ à sa maintenance, son hébergement et son évolution, donnez !</p>
    ...
    <li>
        <div class="bloc_content">
            <a href="/blog/contact" class="link_orange"><img src="/template/default/icon_email.gif" align="absmiddle"
                                                             border="0"/> <b>Contacter le webmaster</b><br/>Faire un
                feedback, proposer une idée ou signaler un problème ?</a>
        </div>
    </li>
...
function CoupDeChapeau()
{
    ...
    if (DB::isError($result_cup)) {
        ...
        if ($pp_cup = $result_cup->fetchRow()) {
            ...
            echo "<div><strong><a href=\"/cup.php?id=" . $pp_cup->id_cup . "&division=1\" class=\"link_orange\">" . $pp_cup->label . "</a></strong><br /><a href=\"/user.php?q=" . urlencode(
                    htmlspecialchars($joueur->login)
                ) . "\" class=\"link_orange\"><img src=\"" . $avatar . "\" vspace=\"4\" height=\"29\" width=\"29\" border=\"0\" align=\"absmiddle\" />&nbsp;" . $joueur->login . "</a></div><hr>";
        }
    }
        ...
        if ($pp_class = $result_class->fetchRow()) {
            ...
            if (DB::isError($result_cup_user)) {
                ...
                if ($cup_user = $result_cup_user->fetchRow()) {
                    ...
                    echo "<div><strong><a href=\"/class.php?id=" . $pp_class->id_class . "\" class=\"link_orange\">" . $pp_class->label . "</a></strong><br /><a href=\"/user.php?q=" . urlencode(
                            htmlspecialchars($joueur->login)
                        ) . "\" class=\"link_orange\"><img src=\"" . $avatar . "\" height=\"29\" width=\"29\" vspace=\"4\" border=\"0\" align=\"absmiddle\" />&nbsp;" . $joueur->login . "</a></div><hr>";
                }
            }
        }
    }
...
function LiveScore()
{
    ...
    if (count($matches_en_cours) || count($matches_termine) || count($matches_a_venir)) $content = getBlocLeft(
        '<span style="float:right;">' . date($txtlang[AFF_DATE_TIME_PHP]) . '</span>En direct',
        'blue',
        $content,
        '',
        false
    ...
    foreach ($matches as $id_match => $match) if ($match->score != "R-R") {
        ...
        if (!(substr($tabd[0], 0, 1) == '-' || $tabd[0] * 1 > 1 || $tabd[0] * 1 == 1 && $tabd[1] * 1 >= 49)) {
            ...
            if ($match->score != "R-R") $content .= '<div style="float:left; width:150px; background:#f5f5f5">
			    <a class="link_button_urgent" href="' . $url_info_match . '" style="float:left; width:' . round(
                    ($minutes * 150 / 90) - 16
                ) . 'px;">';
...
function CurrentMatches($id_matches = false, $options = false)
{
    ...
    if (DB::isError($result_matches)) {
        ...
        while ($pp_matches = $result_matches->fetchRow()) {
            ...
            $dayweek = $dayweek - 2;
            $list_matches[$pp_matches->id_matches]->date_first_match_libelle = get_date_complete(
                    $dayweek,
                    $pp_matches->date_first_match_day,
                    $pp_matches->date_first_match_month - 1,
                    $pp_matches->date_first_match_year
    ...
    $nbmatchestotal = 0;
    foreach ($list_matches as $pp_matches) if (!$pp_matches->save_yet && substr(
            $pp_matches->diff_date_last_match,
            0,
            1
        ) == '-'
    ) {
    ...
    foreach ($list_matches as $pp_matches) if (!$pp_matches->save_yet && substr(
        ...
        $nbmatches++;
        $cssfirstmatches = 'style="font-size:21px;"';
        ...
        if ($pp_matches->id_cup_matches && !$options['forcup']) {
            ...
            if($nbmatches > 1) $cssfirstmatches = '';
            ...
            $content .= "<div id=\"current_matches_" . $nbmatches . "\" class=\"" . $class_line . "\" " . ($nbmatches > $nbmatches2show ? 'style="display:none;"' : '') . "><table width=\"100%\" border=\"0\" cellspacing=\"0\" cellpadding=\"4\"><tr>
				<td valign=\"top\" width=\"1%\"><a href=\"pronostiquer.php?id=" . $pp_matches->id_matches . "\"><img src=\"http://www.pronoplus.com/template/default/" . $pp_matches->image . "\" class=\"preview_matches_image\" border=\"0\" style=\"border:solid 3px #eee;\" alt=\"" . htmlspecialchars(
                    formatDbData($pp_matches->label)
                ) . "\" /></a></td>
				<td  valign=\"top\" width=\"99%\"><table width=\"100%\" border=\"0\" cellspacing=\"0\" cellpadding=\"0\"><tr><td valign=\"top\"><h3><a href=\"pronostiquer.php?id=" . $pp_matches->id_matches . "\" ".$cssfirstmatches.">" . formatDbData(
                    $pp_matches->label
                ) . "</a><br /><span class=\"red\">" . format_diff_date($pp_matches->diff_date_first_match) . "</span></h3></td>
				<td align=\"right\">";

            $tabb = explode(':', $pp_matches->diff_date_first_match);
            $content .= "<a href=\"pronostiquer.php?id=" . $pp_matches->id_matches . "\" class=\"" . ($tabb[0] < -24 ? "link_button" : "link_button_urgent") . "\" ".$cssfirstmatches.">Pronostiquer</a>";
            ...
            $content .= pp_comments_nb_afficher(
                '/pronostiquer.php?id=' . $pp_matches->id_matches . '#comments',
                'classj',
                $pp_comments_id_type
            );
    ...
    if (!$options["liste_non_joues"]) {
        ...
        foreach ($list_matches as $pp_matches) if ($pp_matches->save_yet || substr(
            ...
            if ($pp_matches->id_cup_matches && !$options['forcup']) {
                ...
                $content_list .= "<tr class=\"" . $class_line . " parent_show_on_hover\">" . CurrentCup(
                        $options
                    ) . "</tr>";
                ...
                $content_list .= '<small class="red">' . format_diff_date(
                        $pp_matches->diff_date_first_match
                    ) . '</small>';
...
//                if (!$pp_matches->id_cup_matches) {
//                    $content_list .= '<div class="show_on_hover">Compte pour : ' . getMatchesClass(
//                            $pp_matches->id_matches,
//                            true
//                        );
//                    $content_list .= '<br /><strong>' . getNbUsersMatches(
//                            $pp_matches->id_matches
//                        ) . '</strong> joueurs</div>';
//                }
...
function ClassMatches($options = array())
{
    ...
    $content .= "<tr class=\"" . $class_line . " parent_show_on_hover\">" . CurrentCup(
            array('miniature' => true, 'simple' => true)
        ) . "</tr>";
    $content .= '</table><br />';
        ...
        if ($result_matches->numRows()) {
            ...
            while ($pp_matches = $result_matches->fetchRow()) {
                ...
                if (!$options['return_data']) {
                    ...
                    $content .= '<td width="70%" valign="top"><h3><a href="classj.php?id=' . $pp_matches->id_matches . '">' . formatDbData(
                            $pp_matches->label
                        ) . '</a></h3>';
                    // commentaires ?
                    $pp_comments_id_type = substr($pp_matches->date_first_match, 0, 4) . $pp_matches->id_matches;
                    $content .= pp_comments_nb_afficher(
                        '/classj.php?id=' . $pp_matches->id_matches . '#comments',
                        'classj',
                        $pp_comments_id_type
                    );

                    $content .= '<div class="show_on_hover">Compte pour : ' . getMatchesClass(
                            $pp_matches->id_matches,
                            true
                        ) . '<br />'
                    ...
                    $content .= isset($class_user['class']) ? '<a href="/classj.php?id=' . $pp_matches->id_matches . '&rech_jpseudo=' . urlencode(
                            $user->login
                        ) . '&search_joueur=1" class="link_orange" style="display:block; width:100%">' . $class_user['class'] . '</a>' : '-';
                    $content .= '</td>';
                    $content .= '<td align="center">';
                    $content .= isset($class_user['nb_points']) ? '<a href="classj.php?id=' . $pp_matches->id_matches . '&rech_jpseudo=' . urlencode(
                            $user->login
                        ) . '&search_joueur=1" class="link_orange" style="display:block; width:100%">' . $class_user['nb_points'] . '</a>' : '-';
                    $content .= '</td>';
                    $content .= '</tr>';

                    if ($id_resultat == 3) {
                        $content .= '<tr id="plus_de_resultats"><td colspan="4" style="padding:4px;"><div class="div_link_plusde"><a href="#" onclick="return OpenOrCloseResultats(true);" class="link_plusde">+ de résultats (' . ($result_matches->numRows(
                                ) - 3) . ')</a></div></td></tr>';
                    }
...
function getMatchesClass($id_matches, $noparagraph = false)
{
    ...
    if (DB::isError($result_class)) {
        ...
        while ($pp_class = $result_class->fetchRow()) {
            $label_class .= ($label_class != "" ? ", " : "") . "<a href=\"/class.php?id=" . $pp_class->id_class . "\" class=\"link_orange\">" . formatDbData(
                    str_replace('Classement', '', $pp_class->label)
                ) . "</a>";
        }
    }
    ...
    if (DB::isError($result_class)) {
        ...
        while ($pp_cup = $result_cup->fetchRow()) {
            $label_class .= ($label_class != "" ? ", " : "") . "<a href=\"/cup.php?id=" . $pp_cup->id_cup . "\" class=\"link_orange\">" . formatDbData(
                    $pp_cup->label
                ) . "</a>";
        }
    }
    ...
    if (!$noparagraph) $label_class = "<p>" . ($label_class ? "Cette grille est comptabilisée dans les classements Prono+ suivants : " . $label_class : $justforfun) . "</p>";
    ...
    <table width="100%" border="0" cellpadding="0" cellspacing="0">
        <td width="5%" nowrap="nowrap">
            <?php if (($sqldep - $nbaff) >= 0) { // suivant // précédent
                ...
                <a href="/<?php echo $pagego . "?sqldep=$sqlgoto" . $extension ?>" class="link_button"><img
                        src="/template/default/arrow_left.png" border="0" align="absmiddle"/>&nbsp;Vers&nbsp;les&nbsp;premiers</a>
            <?php } ?>
        </td>
            ...
            <td width="90%" align="center">
                ...
                <select name="sqldep">
                    ...
                    for ($i = 1; $i <= $nbpage; $i++) {
                        ...
                        <option
                            value="<?php echo $ldeb; ?>"<?php if ($ldeb == $sqldep) echo " selected=\"selected\" style=\"font-weight: bold; background-color: #eeeeee;\""; ?>><?php echo ($ldeb + 1) . " - " . ($ldeb + $nbaff); ?></option>
                    <?php } ?>
                </select>
        ...
        <?php if (($sqldep + $nbaff) < $nb_element) {
            $sqlgoto = $sqldep + $nbaff; ?>
            <a href="/<?php echo $pagego . "?sqldep=$sqlgoto" . $extension ?>" class="link_button">Vers&nbsp;les&nbsp;derniers&nbsp;<img
                    src="/template/default/arrow_right.png" border="0" align="absmiddle"/></a>
        <?php } ?>
        </td>
...
function getLastPostFromForum()
{
    ...
    <table width="100%" border="0" cellspacing="1" cellpadding="4">
		...
		*/
        ...
        <?php
        $resmsg = mysql_query(
            "SELECT forum.*, forum_theme.label AS label_theme, forum_theme.url AS url_theme FROM forum INNER JOIN forum_theme ON forum_theme.id_forum_theme=forum.id_forum_theme
        ...
        while ($lmsg = mysql_fetch_array($resmsg)) {
            ...
            <tr class="<?php echo $class = ($class != 'ligne_blanche' ? 'ligne_blanche' : 'ligne_grise'); ?>">
                <td width="65%">
                    <h3><a href="<?php echo $link; ?>" class="link_orange">
                            ...
                            <img src="/template/default/last.gif" alt="Aller au dernier message" align="absmiddle"
			...
			<?php } */
                ...
                <td width="35%">
                    <a href="/user.php?q=<?php echo urlencode(htmlspecialchars($lnmsg["login"])) ?>"
                       class="link_orange">
                        <?php
                        if ($avatar = getAvatar(
                            $lnmsg["id_user"],
                        ...
                        <?php } else { ?>
                            <img src="/template/default/_profil.png" height="30" width="30" border="0"
                                 style="float:left; margin-right:2px;"/>
                        <?php } ?>
                        <?php echo $lnmsg["login"]; ?></a><br/><span style="font-size:10px;"><?php echo formatdateheure(
                            $lmsg["dateder"]
                        ) ?></span>
        ...
        <tr>
            <th align="right" colspan="2">
                <span style="float:left"><a href="/forum_recherche.php" class="link_orange"><img
                            src="/template/default/search.png" border="0" align="absmiddle"/> Rechercher</a></span>
        ...
        <tr>
            <th colspan="2" align="right"><a class="link_orange" href="#"
                                             onclick="$('selection_theme_forum').show(); return false;"><img
                        src="/template/default/comment_new.gif" height="16" width="16" alt="Ouvrir au sujet" border="0"
                        align="absmiddle"> <b>Ouvrir un sujet</b></a>
                <?php
                ...
                if ($pp_cup_matches = $result_matches->fetchRow()) {
                    ...
                    if (DB::isError($result_cup_user)) {
                        ...
                        if ($cup_user = $result_cup_user->fetchRow()) {
                            ...
                            $content .= "<h3><a href=\"cup.php?id=" . $pp_cup->id_cup . "\">" . formatDbData(
                                    $pp_cup->label_cup
                                ) . " - " . getCupTourLabel($pp_cup_matches->number_tour) . "</a></h3>";


                            $first_rank = ($cup_user->cup_sub - 1) * $nbjoueurs + 1;
                            $content .= "<div>Vous êtes qualifié pour la <strong>" . getCupDivisionLabel(
                                    $cup_user->cup_sub
                                ) . "</strong></div>";
                            ...
                            if (!$options["simple"]) {
								...
								<a href=\"/user.php?q=" . urlencode(
                                        htmlspecialchars($opponent->login)
                                    ) . "\" class=\"link_button\"><img src=\"" . $avatar . "\" height=\"29\" width=\"29\" border=\"0\" align=\"absmiddle\" />&nbsp;<strong>" . $opponent->login . "</a></strong></a>";
    ...
    if (DB::isError($result)) {
        ...
        while ($pp_class = $result->fetchRow()) {
            ...
            if (isset($pp_class_user->evolution)) {
                ...
                $evolution = '<a href="/classement-evolution.php?id=' . $pp_class->id_class . '" class="link_orange" title="Voir l\'évolution au ' . htmlspecialchars(
                        formatDbData($pp_class->label)
                    ) . '">' . $evolution . '</a>';
            }

            // affichage ligne
            $html .= '<tr class="' . $class_line . '" onmouseover="this.className=\'ligne_rollover\'" onmouseout="this.className=\'' . $class_line . '\'">
				<td><a href="/class.php?id=' . $pp_class->id_class . '" class="link_orange" title="Voir le ' . htmlspecialchars(
                    formatDbData($pp_class->label)
                ) . '">' . formatDbData($pp_class->label) . '</a></td>
				<td style="text-align:center">' . ($pp_class_user->class ? '<a href="/class.php?id=' . $pp_class->id_class . '&rech_jpseudo=' . htmlspecialchars(
                        $userprofil->login
                    ) . '&search_joueur=1" class="link_orange" title="Aller à mon classement">' . $pp_class_user->class . '</a>' : '-') . '</td>
				<td style="text-align:center">' . $evolution . '</td>
				<td style="text-align:center">' . ($pp_class_user->nb_points ? $pp_class_user->nb_points : '-') . '</td>';

            if (!$options[affichage_simple]) {
                $html .= '<td style="text-align:center">' . ($pp_class_user->nb_matches ? $pp_class_user->nb_matches . ' / ' . $pp_match->NBMATCH . '<br /><span class="petitgris">' . round(
                            100 * $pp_class_user->nb_matches / $pp_match->NBMATCH
                        ) . '%</span>' : '-') . '</td>
					<td style="text-align:center">' . ($pp_class_user->nb_score_ok ? $pp_class_user->nb_score_ok . '<br /><span class="petitgris">' . round(
                            100 * $pp_class_user->nb_score_ok / $pp_class_user->nb_matches
                        ) . '%</span>' : '-') . '</td>
					<td style="text-align:center">' . ($pp_class_user->nb_result_ok ? $pp_class_user->nb_result_ok . '<br /><span class="petitgris">' . round(
                            100 * $pp_class_user->nb_result_ok / $pp_class_user->nb_matches
                        ) . '%</span>' : '-') . '</td>';
            }
            $html .= '</tr>';
        }
    ...
    if (DB::isError($result)) {
        ...
        if ($result_matches->numRows()) {
            ...
            while ($pp_matches = $result_matches->fetchRow()) {
                ...
                $html .= '<td colspan="2"><a href="/classj.php?id=' . $pp_matches->id_matches . '"title="Aller au classement" class="link_orange"><img src="/template/default/' . $pp_matches->image . '" style="border:solid 3px #eee;" height="30" align="absmiddle" /> ' . formatDbData(
                        $pp_matches->label
                    ) . '</a></td>';
...
{
    return '<iframe src="http://www.facebook.com/plugins/like.php?href=http%3A%2F%2Fwww.pronoplus.com%2F' . urlencode(
        $url
    ) . '&amp;layout=' . $layout . '&amp;show_faces=true&amp;width=' . $width . '&amp;action=like&amp;font=verdana&amp;colorscheme=light" scrolling="no" frameborder="0" allowTransparency="true" style="border:none; overflow:hidden; width:' . $width . 'px; height:25px; display:inline;"></iframe>';
}
