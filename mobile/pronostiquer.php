...
?>
    <ul id="msgbugsafari" class="pageitem">
        <li class="textbox">
            <div>BUG sur Safari Mobile : impossible de pronostiquer... En attendant que ce bug soit résolu, essayez
                d'installer Chrome sur votre mobile ;)
            </div>
        </li>
    </ul>
                ...
                <li class="textbox">
                    <table width="100%" cellpadding="2" cellspacing="1">
                        ...
                        <tr<?php echo $cup_user->id_user_host == $cup_user->id_user_won ? ' style="background:yellow"' : ''; ?>>
                            <td align="center" width="10%"><img src="<?php echo $avatar_host; ?>" height="30" width="30"
                                                                border="0" align="absmiddle"/></td>
                            <td width="50%"><?php echo $cup_user->login_host; ?></td>
                            <td align="center"
                                width="40%"><?php echo $cup_user->host_class . '<sup>' . ($cup_user->host_class > 1 ? 'ème' : 'er') . '</sup>'; ?></td>
                        </tr>
                        <tr<?php echo $cup_user->id_user_visitor == $cup_user->id_user_won ? ' style="background:yellow"' : ''; ?>>
                            <td align="center"><img src="<?php echo $avatar_visitor; ?>" height="30" width="30"
                                                    border="0" align="absmiddle"/></td>
                ...
                <table width="100%" border="0" cellpadding="2" cellspacing="1">
                    ...
                    if (is_array($matches)) foreach ($matches as $i => $match) {
                        ...
                        if (substr($match->diff_date_match, 0, 1) == '-') {
                            ?>
                            <input type="hidden" name="id_match[<?php echo $match->id_match ?>]"
                                   value="<?php echo $i ?>"/>
                            <tr>
                                <td align="right"><span
                                        class="overflowed"><?php echo formatDbData($match->team_host_label) ?></span>
                                </td>

                                <td align="center" nowrap="nowrap">
                                    <input id="score_match_<?php echo $i ?>" name="score_match[<?php echo $i ?>]"
                                           type="hidden"
                                           value="<?php echo $score[$match->id_match]->score ? $score[$match->id_match]->score : '0-0'; ?>"/>
                                    <select id="score_team_host_<?php echo $i ?>" onchange="setScore(<?php echo $i ?>)">
                                        <?php
                                        for ($j = 0; $j <= 9; $j++) {
                                            ?>
                                            <option
                                            value="<?php echo $j; ?>" <?php echo substr($score[$match->id_match]->score, 0, 1) == $j ? 'selected="selected"' : ''; ?>><?php echo $j; ?></option><?php
                                        }
                                        ?>
                                    </select>
                                    <select id="score_team_visitor_<?php echo $i ?>"
                                            onchange="setScore(<?php echo $i ?>)">
                                        <?php
                                        for ($j = 0; $j <= 9; $j++) {
                                            ?>
                                            <option
                                            value="<?php echo $j; ?>" <?php echo substr($score[$match->id_match]->score, 2, 1) == $j ? 'selected="selected"' : ''; ?>><?php echo $j; ?></option><?php
                                        }
                                        ?>
                                    </select>
                                </td>
                                ...
                                <td><span
                                        class="overflowed"><?php echo formatDbData($match->team_visitor_label) ?></span>
                                </td>

                                <td align="right">
                                    <select id="mise_match_<?php echo $i ?>" name="mise_match[<?php echo $i ?>]"
                                            onchange="updateMise();">
                                        <?php
                                        for ($j = 5; $j <= 50; $j++) {
                                            ?>
                                            <option
                                            value="<?php echo $j; ?>" <?php echo !$score[$match->id_match]->pts && $j == 10 || $score[$match->id_match]->pts == $j ? 'selected="selected"' : ''; ?>><?php echo $j; ?></option><?php
                                        }
                                        ?>
                                    </select>
                                </td>
                        ...
                        <?php } else { ?>
                            ...
                            <input type="hidden" name="id_match[<?php echo $match->id_match ?>]"
                                   value="<?php echo $i ?>"/>
                            <input type="hidden" id="mise_match_<?php echo $i ?>" name="mise_match[<?php echo $i ?>]"
                                   value="<?php echo $score[$match->id_match]->pts ? $score[$match->id_match]->pts : '0'; ?>"/>
                            <tr>
